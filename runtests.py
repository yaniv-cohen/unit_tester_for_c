...
# Compile the C code
# Loop through all C code files and compile each one separately
for c_code_file in os.listdir(os.path.dirname(__file__)):
    c_code_file_name_only = c_code_file.split(".c")[0]
    if c_code_file.endswith(".c"):
        compile_status = os.system(f"gcc -std=c99 -Wall -Werror -pedantic-errors -lm {math_lib_flag} {c_code_file} -o {os.path.splitext(c_code_file)[0]}")
        if compile_status != 0:
            print(f"Compilation of {c_code_file} failed")
            exit()
            
    # Loop through all the input files in the tests directory
    for input_file in os.listdir(tests_dir):
        if input_file.endswith("_in.txt") and input_file.startswith(c_code_file_name_only):
            ...
            command = f"hw2 < {input_path} > {output_path}"
